...
def generate_email(probability, input_dict, explanation, surname):
    prompt = f"""You are a manager at DS Bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers.
...
if selected_customer_option:
    ...
    location = st.selectbox(
      "Location", ["Spain", "France", "Germany"],
        index=["Spain", "France", "Germany"].index(
          selected_customer['Geography'].values[0]
        )
    )
    gender = st.radio(
      "Gender", ["Male", "Female"],
  ...
  st.markdown("---")
  st.subheader("Explanation Prediction")
